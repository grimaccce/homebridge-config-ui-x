...
export class AccessoriesService {
  ...
  public saveLayout() {
    ...
    // Remove rooms with no services in both current layout and cache
    const filteredLayout = mergedLayout.filter((room) => {
      const currentRoom = this.rooms.find(r => r.name === room.name)
      const hasServicesInCurrent = currentRoom?.services.length > 0
      const hasServicesInCache = room.services.length > 0

      if (!hasServicesInCurrent && !hasServicesInCache) {
        return false // Remove room
      }

      if (!hasServicesInCurrent && hasServicesInCache) {
        // Move undiscovered services to the Default Room
        const defaultRoom = mergedLayout.find(r => r.name === 'Default Room') || {
          name: 'Default Room',
          services: [],
        }
        defaultRoom.services.push(...room.services)
        if (!mergedLayout.includes(defaultRoom)) {
          mergedLayout.push(defaultRoom)
        }
        return false // Remove room after moving services
      }
    ...
    this.accessoryLayout = filteredLayout.map(room => ({
      name: room.name,
